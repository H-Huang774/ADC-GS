...
import torch 
from scene.dataset import dataFourDGSdataset
...
class Scene:
    def __init__(self, args : ModelParams, gaussians, load_iteration=None, shuffle=True, duration=None, loader=None, testonly=None, opt=None):
        ...
        if loader == "dynerf":
            scene_info = sceneLoadTypeCallbacks["Dynerf"](args.source_path, args.source_path, args.eval, duration=self.maxtime)
            ...
            scene_info = sceneLoadTypeCallbacks["Dnerf"](args.source_path, args.eval, args.extension)
        else:
        ...
        for resolution_scale in resolution_scales:
            ...
            elif loader in ["dnerf"]:
                print("Loading Training Cameras")
                self.train_cameras[resolution_scale] = dataFourDGSdataset(scene_info.train_cameras, args, 'dnerf')
                print("Loading Test Cameras")
                self.test_cameras[resolution_scale] = dataFourDGSdataset(scene_info.test_cameras, args, 'dnerf')
                print("Loading Video Cameras")
                self.video_cameras[resolution_scale] = dataFourDGSdataset(scene_info.video_cameras, args, 'dnerf')
            else: 
        ...
        if loader not in ["nerfies", "dynerf", "dnerf"]:
        ...
        if self.loaded_iter :
            ...
            # self.gaussians.decompress_gaussians(path, npz_path, gpcc_codec_path)
        else:

            self.gaussians.create_from_pcd(scene_info.point_cloud, self.cameras_extent, self.maxtime)
            # self.gaussians.load_prediction_net(os.path.join(self.model_path,
            #                                         "point_cloud",
            #                                         "iteration_" + str(3000),
            #                                        ))
            # self.gaussians.load_ply(os.path.join(self.model_path,
