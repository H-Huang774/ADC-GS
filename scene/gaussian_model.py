...
class GaussianModel:
    ...
    def __init__(self, sh_degree : int, args):
        ...
        self.use_coarse2fine = args.use_coarse2fine
    ...
    def create_from_pcd(self, pcd : BasicPointCloud, spatial_lr_scale : float, time_line: int):
        ...
        rgbs = torch.ones((fused_point_cloud.shape[0], 3), device="cuda")
        ...
        self.rgb = nn.Parameter(rgbs.requires_grad_(True))
    ...
    def training_setup(self, training_args):
        ...
        l = [
            ...
            {'params': [self.rgb], 'lr': training_args.rgb_lr_init, "name": "rgb"},
        ...
        self.rgb_scheduler_args = get_expon_lr_func(lr_init=training_args.rgb_lr_init,
                                                    lr_final=training_args.rgb_lr_final,
                                                    lr_delay_mult=training_args.rgb_lr_delay_mult,
                                                    max_steps=training_args.network_lr_max_steps)
    ...
    def update_learning_rate(self, iteration):
        ...
        for param_group in self.optimizer.param_groups:
            if param_group["name"] == "xyz":
                ...
                param_group['lr'] = lr
            elif param_group["name"] == "rgb":
                lr = self.rgb_scheduler_args(iteration)
                param_group['lr'] = lr  
    ...
    def save_ply(self, path):
        ...
        attributes = np.concatenate((xyz, normals, scale, rgb, ref_embedding, res_embedding), axis=1)
    ...
    def load_ply(self, path):
        ...
        rgb = np.zeros((xyz.shape[0], len(rgb_names)))
        for idx, attr_name in enumerate(rgb_names):
            rgb[:, idx] = np.asarray(plydata.elements[0][attr_name])
        ...
        self.rgb = nn.Parameter(torch.tensor(rgb, dtype=torch.float, device="cuda").requires_grad_(True))
    ...
    def position_embedding(self):
        ...
        for i in range(3):
            ...
            div_term = div_term.to(pos.device)  # 确保 div_term 和 pos 在同一设备上
    ...
    def prune_points(self, mask):
        ...
        self.rgb = optimizable_tensors["rgb"]
    ...
    def densification_postfix(self, new_xyz, new_scaling, new_rgb, new_ref_embedding, new_res_embedding):
        d = {"xyz": new_xyz,  
        "scaling" : new_scaling,
        "rgb" : new_rgb,
    ...
    def densify_and_clone(self, grads, grad_threshold, scene_extent):
        ...
        # new_xyz_voxel = voxelize_sample(new_xyz, voxel_size= 0.001)
        # xyz_voxel = voxelize_sample(self.xyz, voxel_size= 0.001)
        # xyz_voxel_set = set(map(tuple, xyz_voxel))
        # new_xyz_voxel_tuple = [tuple(voxel) for voxel in new_xyz_voxel]  # 将 new_xyz_voxel 转为元组形式
        # unoccupied_voxels = [voxel for voxel in new_xyz_voxel_tuple if voxel not in xyz_voxel_set]


        scaling = repeat(self.scaling, 'n c -> (n k) c', k=self.derive_factor)
        rgb = repeat(self.rgb, 'n c -> (n k) c', k=self.derive_factor)
        ref_embedding = repeat(self.ref_embedding, 'n c -> (n k) c', k=self.derive_factor)
        res_embedding = repeat(self.res_embedding, 'n g c -> (n k) g c', k=self.derive_factor)

        new_scaling = scaling[selected_pts_mask]
        new_rgb = rgb[selected_pts_mask]
        new_ref_embedding = ref_embedding[selected_pts_mask]
        new_res_embedding = res_embedding[selected_pts_mask]
        self.densification_postfix(new_xyz, new_scaling, new_rgb, new_ref_embedding, new_res_embedding)
    ...
    def prune(self, max_grad, min_opacity, extent, max_screen_size, use_mean=False):
        ...
        # anchor_opacity = opacity.reshape(int(opacity.shape[0]/k), k).sum(axis=1).reshape(int(opacity.shape[0]/k), 1)
        # prune_mask = (anchor_opacity < min_opacity*k).squeeze()
        # grouped_prune_mask = prune_mask
    ...
    def add_densification_stats(self, viewspace_point_tensor, update_filter):
        if self.use_significance_weight:
            ...
            significance_weight = significance[update_filter]
        else:
    ...
    def compress_gaussians(self, path, npz_path, gpcc_codec_path):
        ...
        means, scaling_factors_before_exp, rgb_factors = self.xyz, self.scaling, self.rgb
        ...
        means, scaling_factors_before_exp, rgb_factors = means[sorted_indices], scaling_factors_before_exp[sorted_indices], rgb_factors[sorted_indices]
        ref_feats, res_feats = ref_feats[sorted_indices], res_feats[sorted_indices]

        # compress features
        strings = self.network.compress(ref_feats=ref_feats, res_feats=res_feats, scaling_factors_before_exp=scaling_factors_before_exp, rgb_factors=rgb_factors)
    ...
    def decompress_gaussians(self, path, npz_path, gpcc_codec_path):
        ...
        rgb_strings = data_dict['rgb_strings'].tobytes()
        ...
        ref_feats, res_feats, scaling_factors_before_exp = self.network.decompress(
            ...
            scale_strings=scale_strings, rgb_strings=rgb_strings, num_anchor_primitives=means.shape[0])
        ...
        rgb = torch.ones((means.shape[0], 4), dtype=torch.float32, device=self.device)
    ...
    def voxelize(self):
        grid_size = self.voxel_size_compress
        self.xyz = torch.round(self.xyz / grid_size) * grid_size
        torch.cuda.empty_cache()
