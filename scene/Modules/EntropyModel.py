...
class EntropyModel(CompressionModel):
    def __init__(self, ref_feats_dim: int, ref_hyper_dim: int, res_feats_dim: int, res_hyper_dim: int, derive_factor: int, scale_dim:int = 6, quant_step: float = 1) -> None:
        ...
        self.quant_step_rgb = nn.Parameter(torch.ones(3, dtype=torch.float32, device=device) * quant_step * 0.01, requires_grad=True)
        ...
        self.h_s_rgb = ResidualMLP(in_dim=ref_feats_dim, internal_dim=ref_feats_dim, out_dim=2 * 3, num_res_layer=2)
    ...
    def forward(self, y_ref: torch.Tensor, y_res: torch.Tensor, y_scale: torch.Tensor, y_rgb: torch.Tensor):
        ...
        y_ref_hat, y_likelihoods_ref = self.gaussian_conditional(y_ref, scales=F.softmax(scales_hat_ref), means=means_hat_ref)
        ...
        y_res_pre_hat, y_likelihoods_res = self.gaussian_conditional(y_res_pre, scales=F.softmax(scales_hat_res_pre), means=means_hat_res_pre)
        ...
        y_res_dfm_hat, y_likelihoods_res_dfm = self.gaussian_conditional(y_res_dfm, scales=F.softmax(scales_hat_res_dfm), means=means_hat_res_dfm)
        ...
        y_scale_hat, y_likelihoods_scale = self.gaussian_conditional(y_scale, scales=F.softmax(scales_hat_scale), means=means_hat_scale)
        y_scale_hat = y_scale_hat * self.quant_step_scale

        gaussian_params_rgb = self.h_s_rgb(y_ref_hat)
        means_hat_rgb, scales_hat_rgb = torch.chunk(gaussian_params_rgb, dim=-1, chunks=2)
        means_hat_rgb = means_hat_rgb / self.quant_step_rgb
        scales_hat_rgb = scales_hat_rgb / self.quant_step_rgb
        y_rgb = y_rgb / self.quant_step_rgb
        y_rgb_hat, y_likelihoods_rgb = self.gaussian_conditional(y_rgb, scales=F.softmax(scales_hat_rgb), means=means_hat_rgb)
        y_rgb_hat = y_rgb_hat * self.quant_step_rgb
        ...
        likelihoods = {
            ...
            'scales': y_likelihoods_scale, 'rgb': y_likelihoods_rgb
        }
        return y_ref_hat, y_res_hat, y_scale_hat, y_rgb_hat, likelihoods
    
    @torch.no_grad()
    def compress(self, ref_feats: torch.Tensor, res_feats: torch.Tensor, scaling_factors_before_exp: torch.Tensor, rgb_factors: torch.Tensor):
        ...
        gaussian_params_rgb = self.h_s_rgb(y_ref_dec)
        means_hat_rgb, scales_hat_rgb = torch.chunk(gaussian_params_rgb, dim=-1, chunks=2)
        means_hat_rgb = means_hat_rgb / self.quant_step_rgb
        scales_hat_rgb = scales_hat_rgb / self.quant_step_rgb
        rgb_factors = rgb_factors / self.quant_step_rgb
        indices_rgb = self.gaussian_conditional.build_indexes(F.softmax(scales_hat_rgb))
        y_rgb_strings = self.gaussian_conditional.compress(rgb_factors, indices=indices_rgb, means=means_hat_rgb)
        strings = {
            ...
            'scale_strings': y_scale_strings, 'rgb_strings': y_rgb_strings
        }
        return strings
    
    @torch.no_grad()
    def decompress(self, y_ref_strings: bytes, z_ref_strings: bytes, y_res_pre_strings: bytes, z_res_strings: bytes, y_res_dfm_strings: bytes, y_scale_strings: bytes, y_rgb_strings: bytes, N: int):
        ...
        scales_hat_rgb = scales_hat_rgb / self.quant_step_rgb
        indices_rgb = self.gaussian_conditional.build_indexes(F.softmax(scales_hat_rgb))
        y_rgb_dec = self.gaussian_conditional.decompress(y_rgb_strings, indices=indices_rgb, means=means_hat_rgb)
        y_rgb_dec = y_rgb_dec * self.quant_step_rgb

        y_res_dec = torch.cat([y_res_pre_dec,y_res_dfm_dec],dim=2)
        return y_ref_dec, y_res_dec, y_scale_dec, y_rgb_dec
