...
class Networks(CompressionModel):
    ...
    def forward(self, means: torch.Tensor, scaling_factors_before_exp: torch.Tensor,
                ref_feats: torch.Tensor, res_feats: torch.Tensor, rgb_factors: torch.Tensor, skip_quant: bool = False,
        ...
        :param rgb_factors: rgb factors, shape (N, 3).
        ...
        if skip_quant:
            ...
            ref_feats, res_feats, scaling_factors_before_exp, rgb_factors, likelihoods = self.entropy_model(
                y_ref=ref_feats, y_res=res_feats, y_scale=scaling_factors_before_exp, y_rgb=rgb_factors)
        ...
        gaussian_primitives, pred_opacities, coupled_primitive_mask = self.prediction_net(
            means=means, scaling_factors=torch.exp(scaling_factors_before_exp), ref_feats=ref_feats, res_feats=predict_res_feats, rgb_factors=rgb_factors)
    ...
    @torch.no_grad()
    def compress(self, ref_feats: torch.Tensor, res_feats: torch.Tensor, scaling_factors_before_exp: torch.Tensor, rgb_factors: torch.Tensor) -> Dict[str, bytes]:
        """
        ...
        """
        return self.entropy_model.compress(ref_feats=ref_feats, res_feats=res_feats, scaling_factors_before_exp=scaling_factors_before_exp, rgb_factors=rgb_factors)
    ...
    def decompress(self, ref_feats_strings: bytes, ref_hyper_strings: bytes, res_feats_pre_strings: bytes, res_hyper_strings: bytes, res_feats_dfm_strings: bytes,
                   scale_strings: bytes, rgb_strings: bytes, num_anchor_primitives: int) -> Tuple[torch.Tensor, torch.Tensor, torch.Tensor]:
        """
        ...
        return self.entropy_model.decompress(
            ...
            y_scale_strings=scale_strings, y_rgb_strings=rgb_strings, N=num_anchor_primitives)
    ...
    def pred_gaussian_scales(self, means: torch.Tensor, scaling_factors_before_exp: torch.Tensor,
                            ref_feats: torch.Tensor, res_feats: torch.Tensor) -> torch.Tensor:
        """
        PreDict scales of Gaussian primitives, used in adaptive control.
        :param means: means of anchor primitives, shape (N, 3).
        :param scaling_factors_before_exp: scaling factors of anchor primitives before exp activation, shape (N, 6).
        :param ref_feats: reference features, shape (N, C).
        :param res_feats: residual features, shape (N, K, G).
    ...
    def pred_gaussian_opacities(self, means: torch.Tensor, scaling_factors_before_exp: torch.Tensor,
                            ref_feats: torch.Tensor, res_feats: torch.Tensor) -> torch.Tensor:
        """
        PreDict opacity of Gaussian primitives, used in adaptive control.
        :param means: means of anchor primitives, shape (N, 3).
        :param scaling_factors_before_exp: scaling factors of anchor primitives before exp activation, shape (N, 6).
        :param ref_feats: reference features, shape (N, C).
        :param res_feats: residual features, shape (N, K, G).
