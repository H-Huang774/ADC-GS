...
class PredictionNetwork(nn.Module):
    def __init__(self, ref_feats_dim: int, res_feats_dim: int, derive_factor: int) -> None:
        ...
        view_feats_dim = 4  # dimension of view features, 3 for view direction and 1 for view distance + view_feats_dim
    ...
    def forward(self, means: torch.Tensor, scaling_factors: torch.Tensor, ref_feats: torch.Tensor, res_feats: torch.Tensor,
        ...
        # # 假设 means_offset 是一个 PyTorch Tensor
        # means_offset_np = means_offset.detach().cpu().numpy()  # 将 Tensor 转为 NumPy 数组
        # np.savetxt('means_offset.txt', means_offset_np)
        ...
        # means = means[coupled_primitive_mask]
        # scaling_factors = scaling_factors[coupled_primitive_mask]
        ...
        # pred_opacities = pred_opacities[coupled_primitive_mask]

        # print("shape:", means.shape, means_offset.shape, pred_covariance.shape, means_scaling_factor.shape, scaling_factors.shape)
