...
class deform_network(nn.Module):
    def __init__(self, D=8, W=256, min_embeddings=30, max_embeddings=150, num_frames=300, num_cam=None, args=None,):
        ...
        #to be changed
        self.gaussian_embedding_dim = args.position_embedding_dim + int(args.res_embedding_dim / 2) + args.ref_embedding_dim#args.gaussian_embedding_dim
    ...
    def query_time(self, pts, scales, rotations, time_emb, deform_embedding=None, embeddings=None, sh_coef=None, iter=None, feature_out=None, use_coarse_temporal_embedding=False, num_down_emb=30):
        t = time_emb[:,:1]
        h = self.get_temporal_embed(t, num_down_emb)
    
        if type(deform_embedding) == type(None):
            h = torch.cat([h, embeddings], dim=-1)
        else:        
            h = torch.cat([h, deform_embedding], dim=-1)
    ...
    def deform(self, hidden, pts, scales, rotations, opacity, sh_coefs, pos_deform, scales_deform, rotations_deform, opacity_deform, rgb_deform, scale=1., scale_c=1., scale_o=1., coef_s=1.):
        ...
        if not self.args.no_dc:
            ...
            # print("shshape",sh_coefs.shape)
            sh_coefs = sh_coefs + dc * scale_c
        return pts, scales, rotations, opacity, sh_coefs

    def forward(self, point, scales=None, rotations=None, opacity=None, time_emb=None, cam_no=None, deform_embedding=None, embeddings=None, sh_coefs=None, iter=None, num_down_emb_c=30, num_down_emb_f=30):
        ...
        hidden = self.query_time(pts, scales, rotations, time_emb, deform_embedding, embeddings, sh_coefs, iter, self.feature_out_c, self.args.use_coarse_temporal_embedding, num_down_emb=num_down_emb_c).float()        
        pts, scales, rotations, opacity, sh_coefs = self.deform(hidden, pts, scales, rotations, opacity, sh_coefs,\
