...
os.system('echo GPU: $CUDA_VISIBLE_DEVICES')
...
def scene_reconstruction(dataset, opt, hyper, pipe, testing_iterations, saving_iterations, 
    ...
    num_traincams = 1
    print('dataset.type:', dataset.loader)
    ...
    for train_cam in train_cams:
        center = train_cam.camera_center
    ...
    for iteration in range(first_iter, final_iter+1):             
        ...
        camera_center_list = []
        anchor_visibility_filter_list = []
        scales_list = []
        bpp_list = []
        if iteration < opt.compress_start_iter:
            skip_quant_flag = True
        else:
            skip_quant_flag = False
        for viewpoint_cam in viewpoint_cams:
            ...
            render_results = render(viewpoint_cam, gaussians, pipe, background, cam_no=cam_no, iter=iteration, \
                num_down_emb_c=hyper.min_embeddings, num_down_emb_f=hyper.min_embeddings, skip_quant=skip_quant_flag)
            image, viewspace_point_tensor, visibility_filter, radii = render_results.rendered_img, render_results.projected_means, render_results.visibility_mask, render_results.radii
            bpp = render_results.bpp
            bpp_total = sum(v for v in bpp.values())
            ...
            scales_list.append(render_results.scales.prod(dim=1).mean())
        ...
        if  opt.lambda_dssim > 0.:
            ...
            assert not torch.isnan(Lssim).any(), "Lssim contains NaN values"
            assert not torch.isnan(loss).any(), "Loss contains NaN values"
        else:
            loss = Ll1

        if len(bpp_list)!=0:
            bpp = torch.tensor(bpp_list, dtype=torch.float32).mean()
            rate_loss = opt.lambda_rate_control * bpp
            loss = loss + (rate_loss if iteration > opt.rate_control_start_iter else 0.)
        ...
        if prev_num_pts != gaussians.xyz.shape[0]:
            neighbor_sq_dist, neighbor_indices = o3d_knn(gaussians.xyz.detach().cpu().numpy(), 5)
            ...
            prev_num_pts = gaussians.xyz.shape[0]
        emb = gaussians.ref_embedding[:,None,:].repeat(1,5,1)
        emb_knn = gaussians.ref_embedding[neighbor_indices]
        loss += opt.reg_coef * weighted_l2_loss_v2(emb, emb_knn, neighbor_weight)

        with torch.autograd.detect_anomaly():
            loss.backward()
            aux_loss = gaussians.network.aux_loss()
            aux_loss.backward()            
        iter_end.record()
        ...
        if iteration in saving_iterations:
            ...
            with open(os.path.join(args.model_path, 'training_time.txt'), 'a') as file:
                file.write(f'Iteration {iteration}: {total_time_seconds} seconds ... {int(hours)}h {int(minutes)}m {seconds}sec  points: {gaussians.xyz.shape[0]}\n')

        with torch.no_grad():
            # Progress bar
            loss_list_out.append(loss.item())
            psnr_list_out.append(psnr_.cpu().item())
            ema_loss_for_log = 0.4 * loss.item() + 0.6 * ema_loss_for_log
            ema_psnr_for_log = 0.4 * psnr_ + 0.6 * ema_psnr_for_log
            total_point = gaussians.xyz.shape[0]
            if iteration % 100 == 0:
            ...
            if iteration < opt.iterations:
                ...
                gaussians.aux_optimizer.step()
                gaussians.aux_optimizer.zero_grad(set_to_none = True)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--save_iterations", nargs="+", type=int, default=[10000, 20000, 30000, 40000, 50000, 60000, 80000, 100000, 120000])
