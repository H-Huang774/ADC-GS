...
def positional_encoding(x, position_embedding_dim=48):
    frequencies = torch.arange(0, position_embedding_dim // (2 * 3), device=x.device)  # 保证 frequencies 在同一设备上
    div_term = 10000 ** (2 * frequencies / position_embedding_dim)
    pos_encodings = []
    for i in range(3):
        pos = x[:, i].unsqueeze(1)
...
def render(viewpoint_camera, pc : GaussianModel, pipe, bg_color : torch.Tensor, scaling_modifier = 1.0, override_color = None, cam_no=None, iter=None, train_coarse=False, \
    ...
    rgb_factors = pc.get_rgb_factors
    ...
    if (iter <= 8000): 
        ...
        gaussian_primitives, _, _, bpp, ref_feats, deform_res_feats = pc.network(
            ...
            rgb_factors=rgb_factors, skip_quant=skip_quant)  
    ...
    if (iter >= 8000):
        ...
        if pc.use_coarse2fine:
            means3D_final, scales_final, rotations_final, _, _ = pc.deformation(gaussian_primitives.means, gaussian_primitives.scales,
            ...
            gs_position_embedding = positional_encoding(means3D_final, position_embedding_dim=pc.position_embedding_dim)

            _, _, _, opacity_final, colors_precomp = pc.deformation(gaussian_primitives.means, gaussian_primitives.scales,
                gaussian_primitives.rotations, gaussian_primitives.opacities, time, cam_no, gs_position_embedding, None, gaussian_primitives.colors, iter=iter, num_down_emb_c=num_down_emb_c, num_down_emb_f=num_down_emb_f)
        else:
