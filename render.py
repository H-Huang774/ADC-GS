...
def render_set(model_path, name, iteration, views, gaussians, pipeline, background, hyperparam=None):
    ...
    for idx, view in enumerate(tqdm(views, desc="Rendering progress")):
        if type(view.original_image) == type(None):
            if name == 'video':
                ...
                view.load_image()
        time1 = time()
        render_results = render(view, gaussians, pipeline, background, iter=iteration, num_down_emb_c=num_down_emb_c, num_down_emb_f=num_down_emb_f,skip_quant=True)
        rendering = render_results.rendered_img
        time2 = time()
    ...
    #save
    fps_rendering = (len(views)-1)/total_time
    fps_file_path = os.path.join(model_path, name, "ours_{}".format(iteration), 'fps.txt')
    os.makedirs(os.path.dirname(fps_file_path), exist_ok=True)

    # 将 FPS 写入文件
    with open(fps_file_path, 'w') as fps_file:
        fps_file.write(f"FPS: {fps_rendering:.2f}\n")  # 保留两位小数
